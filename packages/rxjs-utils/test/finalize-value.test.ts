...
import { finalizeWithValue } from '../src'
...
describe('finalizeWithValue', () => {
	it('passes values through and calls with the last value on complete', async () => {
		const spy = vi.fn()
		const out = of('a', 'b', 'c').pipe(finalizeWithValue(spy))
	...
	it('calls with the last value on error', async () => {
		...
		const out = src.pipe(finalizeWithValue(spy))
	...
	it('calls on unsubscribe (e.g., via take)', async () => {
		const spy = vi.fn()
		const out = of('x', 'y', 'z').pipe(finalizeWithValue(spy), take(1))
	...
	it('passes undefined when there were no emissions', async () => {
		const spy = vi.fn()
		const out = EMPTY.pipe(finalizeWithValue(spy))
